...
# JTAG Test Clock (TCK)
# Period = 100 ns → Frequency = 10 MHz
create_clock -name tck -period 100 [get_ports tck]

# Apply clock uncertainty to JTAG clock
set_clock_uncertainty -setup 0.2 [get_clocks tck]
set_clock_uncertainty -hold  0.1 [get_clocks tck]


################################################################################
# CLOCK DOMAIN RELATIONSHIP
#
# Functional clock and JTAG clock are independent.
# STA should NOT analyze timing paths crossing these domains.
################################################################################
...
set_input_delay 1.0 -clock tck [get_ports tdi]
set_input_delay 1.0 -clock tck [get_ports tms]

# Uncomment if TRST exists in design
# set_input_delay 1.0 -clock tck [get_ports trst]
...
# JTAG outputs
set_output_delay 1.0 -clock tck [get_ports tdo]
set_output_delay 1.0 -clock tck [get_ports tdo_paden_o]